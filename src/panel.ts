...
import {
  aplicarSync, lerNotas, lerPrefs, lerProgresso, log, reenfileirar, stats,
  type OpSync,
} from "./db.ts";
...
function iniciar(db: Database, porta: number): void {
  ...
  const servidor = Bun.serve({
    ...
    async fetch(req) {
      ...
      // Toda escrita do painel entra por aqui, em lote: marcação, posição do
      // vídeo, anotação e preferência. O navegador enfileira no localStorage e
      // só tira da fila o que este endpoint confirmar — é o que faz uma piscada
      // do túnel deixar de engolir o que a pessoa marcou.
      //
      // Reenviar o mesmo lote é inofensivo: toda operação é "deixe assim", e
      // não "some mais um". Por isso o cliente pode tentar de novo sem pensar.
      if (rota === "/api/sync" && req.method === "POST") {
        ...
        const r = aplicarSync(db, corpo.ops);
        // Devolve o estado fresco junto: quem acabou de sincronizar já recebe o
        // que os outros aparelhos gravaram, sem uma segunda viagem.
        return Response.json({
          ok: true, ...r,
          progresso: lerProgresso(db), notas: lerNotas(db), prefs: lerPrefs(db),
        });
      }
